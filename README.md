...
`capture3d --assets FILE` charge le manifeste strict `aetherion.assets3d/v1`. Chaque entrée contient `id`, `path`, `type` (`mesh` ou `material`), `size` et `checksum` FNV-1a 64 bits. Les fichiers sont des documents JSON stricts `aetherion.mesh3d/v1` (`mesh`) ou `aetherion.material3d/v1` (`material`) correspondant directement aux types publics de `render3d`. Les chemins sont relatifs au manifeste et confinés à son dossier ; traversals, liens sortants, tailles/checksums incorrects, quotas et doublons sont rejetés. Le chargement est concurrent, puis collecté dans une `BTreeMap` et fusionné en ordre canonique. Une collision avec une ressource inline est une erreur ; les objets existants peuvent référencer les IDs externes.
