...
Le prototype temps réel s'utilise avec `cargo run --features render-gpu -- gpu-demo --scene FILE [--assets FILE] [--frames N]`. Avec `--frames`, la fenêtre se ferme automatiquement et publie le rapport `aetherion.gpu-demo/v1`, ce qui permet un smoke test borné ; sans cette option, elle reste ouverte jusqu'à fermeture utilisateur. `gpu-benchmark --scene FILE --frames N` publie séparément `aetherion.gpu-benchmark/v1` avec le temps mural, le FPS moyen en milli-FPS, l'adaptateur utilisé, le nombre d'objets visibles/rejetés et les draw calls. Le backend calcule et valide une AABB locale entière par mesh, transforme les huit coins avec les mêmes opérations entières que la scène, puis applique le culling orthographique aux objets avant upload GPU. La géométrie locale préparée est conservée dans les buffers GPU du renderer par couple mesh/matériau/texture, puis les objets visibles sont rendus avec un instance buffer et un draw indexed instancié par lot. Les meshes sans normales dupliquent uniquement leurs sommets par face pour conserver le flat-shading historique ; les meshes avec normales réutilisent leurs sommets et leurs indices. Ces valeurs servent au suivi de performance uniquement : elles ne sont ni des sorties déterministes ni des entrées de simulation. Il partage la validation et la résolution d'assets de `capture3d`, mais les captures reproductibles restent produites par le chemin CPU.
