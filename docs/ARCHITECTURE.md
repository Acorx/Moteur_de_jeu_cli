...
Le backend initialise une surface, choisit un format sRGB et le mode FIFO lorsque le pilote le propose, puis rend un pipeline de triangles colores avec depth buffer `Depth24Plus`. Les meshes peuvent fournir des normales/UV quantifies ; les normales sont transformees dans l'espace de presentation et interpolées par le GPU, avec repli vers la normale de face pour les scenes historiques. La préparation calcule une AABB entière sur les sommets référencés de chaque mesh, transforme ses huit coins avec les opérations entières de la scène, puis rejette les objets hors du volume orthographique avant tout upload. Les objets visibles sont ensuite regroupés par `(mesh, material, texture)` : chaque lot possède une géométrie locale conservée dans des vertex/index buffers pendant la durée du renderer, ainsi qu'un instance buffer contenant les matrices modèle et normale ; un unique draw indexed instancié couvre toutes ses instances. Les meshes sans normales dupliquent leurs sommets par face afin de préserver le flat-shading, tandis que les meshes avec normales réutilisent la géométrie indexée. Les compteurs `objects`, `culled_objects` et `draw_calls` sont publiés dans les rapports GPU. Le fragment shader applique un éclairage directionnel fixe avec une composante ambiante, sans prétendre à une illumination déterministe cross-device. Les erreurs de surface sont classées (`Lost`, `Outdated`, `Timeout`, `OutOfMemory`) et la boucle redimensionne la surface sans interrompre la simulation. `--frames N` borne la boucle à un million de frames maximum et publie `aetherion.gpu-demo/v1` avec dimensions, nombre de frames effectivement présentées, triangles et statistiques de préparation. `gpu-benchmark --scene FILE --frames N` réutilise la même boucle et publie `aetherion.gpu-benchmark/v1` avec temps mural, milli-FPS et nom de l'adaptateur. Ce rapport est volontairement hors déterminisme et ne remplace pas les captures CPU.
...
Les textures externes sont des entrees binaires `texture` du manifeste `aetherion.assets3d/v1`. `resolve_assets_with_textures` les garde hors de `Scene3d` et les transmet uniquement au backend GPU. Le CPU peut donc valider le meme projet et produire une capture reproductible sans decoder d'image ; le GPU decode PNG/JPEG, refuse les images hors quotas et cree un bind group par texture utilisee. Les lots sont groupes par ID de texture afin que chaque draw possede un bind group stable.

Le cache optionnel `--cache-dir` se situe hors des formats de scene et de simulation. Chaque entree est clee par `(type, checksum FNV-1a, CACHE_FORMAT_VERSION, IMPORTER_VERSION)`. Le source reste lu et verifie avant consultation du cache ; les octets caches sont donc une acceleration de parsing, pas une nouvelle autorite. Les publications sont atomiques et les erreurs de cache se replient sur le chemin source.
